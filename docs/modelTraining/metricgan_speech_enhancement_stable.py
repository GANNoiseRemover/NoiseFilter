# -*- coding: utf-8 -*-
"""metricgan_speech_enhancement_stable.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1VsITwiD3m3KsTyhFBLlXzsaJBWJ-QSOD
"""
...
# NOTE: The "Drive already mounted" message is NOT an error. It's just Colab confirming the connection.

# ----------------- Parameters -----------------
sr = 16000
# 🚀 segment_length increased to 32000 (2 seconds)
segment_length = 16000
epochs = 20
batch_size = 16
# IMPORTANT: File paths confirmed to be correct for your Drive structure
clean_folder = "/content/drive/MyDrive/Denoiser/clean_testC"
noisy_folder = "/content/drive/MyDrive/Denoiser/noise_testC"
output_folder = "/content/drive/MyDrive/Denoiser/denoised_new"
CHECKPOINT_DIR = "/content/drive/MyDrive/Denoiser/checkpoints"
LOG_FILE = "/content/drive/MyDrive/Denoiser/training_log.csv"
METRIC_FILE = "/content/drive/MyDrive/Denoiser/epoch_metrics.csv"

METRIC_WEIGHT_INITIAL = 0.05
RECONSTRUCTION_WEIGHT = 1.0
METRIC_RAMPUP_EPOCHS = 20

# 2. DEFINED RAMPING STEP AND FREQUENCY FOR GENTLER INCREASE
METRIC_RAMP_STEP = 0.01
METRIC_RAMP_FREQ = 5 # Increase by 0.01 every 5 epochs after initial ramp
...
# Implemented ExponentialDecay schedule for the Generator's learning rate
initial_g_lr = 1e-4
g_lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
    initial_g_lr,
    decay_steps=len(pairs) * 10,  # Decay every 10 full passes through the data
    decay_rate=0.98,
    staircase=True)

# D_optimizer remains fixed with gradient clipping
d_optimizer = Adam(learning_rate=5e-5, clipnorm=1.0)

# G_optimizer uses the new schedule with gradient clipping
g_optimizer = Adam(learning_rate=g_lr_schedule, clipnorm=1.0)
...
# ----------------- Logg files -----------------
if not os.path.exists(LOG_FILE) or last_epoch == 0:
    with open(LOG_FILE, "w") as f:
        f.write("epoch,avg_g_loss,avg_d_loss,avg_pesq,avg_stoi\n")

if not os.path.exists(METRIC_FILE) or last_epoch == 0:
...
for epoch in range(last_epoch + 1, epochs + 1):
    ...
    # Calculate current metric weight
    if epoch <= METRIC_RAMPUP_EPOCHS:
        metric_weight_current = (epoch / METRIC_RAMPUP_EPOCHS) * METRIC_WEIGHT_INITIAL
    else:
        increment_steps = (epoch - METRIC_RAMPUP_EPOCHS) // METRIC_RAMP_FREQ
        metric_weight_current = METRIC_WEIGHT_INITIAL + increment_steps * METRIC_RAMP_STEP
        metric_weight_current = min(metric_weight_current, 1.0)

    # 🚀 FIX: Correctly call the schedule object to get the current learning rate
    g_lr_current = g_lr_schedule(g_optimizer.iterations).numpy()

    # Updated print statement to show scheduled G_LR
    print(f"  Metric weight: {metric_weight_current:.4f}, Reco weight: {RECONSTRUCTION_WEIGHT:.1f}")
    print(f"  G LR: {g_lr_current:.1e}, D LR: {d_optimizer.learning_rate.numpy():.1e}")
    ...
    for clean_path, noisy_path in pairs:
        ...
        # Handling the last partial batch (code logic remains the same)
        if batch_clean:
        ...
        # Calculating metrics for the current file (EVALUATION)
    ...
    # Only process a small subset for efficiency if the dataset is large
    for _, noisy_path in pairs[:5]: # Processing first 5 files for quick checks
        noisy_audio = load_audio(noisy_path)
    ...
    print(f"✅ Saved {len(pairs[:5])} denoised files to {epoch_output_dir}")
